...
def forecasting(request):
    if request.POST:
       model = pickle.load(open('xgboostFile.pkl', 'rb'))
       modelL = pickle.load(open('lightgbm.pkl', 'rb'))
       modelS = pickle.load(open('gradientboostmodel.pkl', 'rb'))
       ...
       output = int(output)
       outputL = modelL.predict(data)
       #arrayData = pd.DataFrame(data)
       # print("Data type:", type(arrayData))
       # print("Data shape:", arrayData.shape)
       # print("ModelS type:", type(modelS))
       #print("Data index:", data.index)
       #start_date = pd.to_datetime('2024-01-01')
        # Check `start` parameter
       #start_position=len(arrayData)
       #end_position=start_position + len(arrayData) - 1
       #print("Start parameter:", start_date)

        #updatedData = arrayData.to_frame().T
        
       outputSSeries = modelS.predict(data)
       print(outputSSeries)
        #output_plus = output + 16;
       outputL = int(outputL)
        #outputS = int(outputS)
       # s = np.median(outputSSeries)
       outputS = int(outputSSeries)
       if output < 0:
           output = 0
       context = {'output': output,'outputL': outputL, 'outputS': outputS, 'category':category,'cuisine':cuisine,'week':week,'checkout_price':checkout_price,'base_price':base_price,'emailer':emailer,'homepage':homepage,'city':city,'region':region,'op_area':op_area,'center_type':center_type}
